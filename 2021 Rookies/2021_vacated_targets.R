...
for (i in 1:nrow(teams)) {
  # load stats for 2020 season
  stats2020 <- load_player_stats(seasons = 2020) %>%
  ...
  roster_2021 <- load_rosters(seasons = 2021) %>%
  ...
  wr_2020 <- right_join(roster_2020, stats2020, by="player_id") %>%
    filter(!is.na(position))
  
  
  lost_players <- left_join(wr_2020, roster_2021, by="player_id") %>%
...
write.csv(vac_tar, "2021_targets.csv", row.names = FALSE)
